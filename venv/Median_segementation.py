...
class MMCQ(object):
    ...
    def getPixHisto(self):
        ...
        for y in range(self.h):
            for x in range(self.w):
                r = self.pixData[y, x, 0] >> self.rshift   # >>位运算，右移，例：100->010,也就是十进制的4变成2，
                g = self.pixData[y, x, 1] >> self.rshift   # >>位运算，右移在二进制相当于除以二，十进制除以十。
    ...
    def iterCut(self, maxColor, boxQueue, vol=False):
        ...
        while True:
            ...
            if vol:
                vbox1.priority *= int(vbox1.vol)
            boxQueue.put((vbox1.priority, vbox1))
            if vbox2 is not None:
                ncolors += 1
                if vol:
                    vbox2.priority *= int(vbox2.vol)
    ...
    def boxAvgColor(self, vbox):
        ...
        themep=list(map(lambda x: int(x), avgs))
        themep.append(ntot)
        return themep
